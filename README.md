...
```
일반 모드에서
1. 'q'를 누르고 a~z 사이의 알파벳을 누르면 매크로 recording 시작
2. 반복을 위한 원하는 동작 실행
3. 일반 모드로 돌아와서 'q'를 누르면 매크로 recording 끝
4. 매크로를 사용하려면 일반 모드에서 @+a~z 사이의 알파벳을 입력하면 됨
```

* __전에 실행한 매크로를 실행하는 방법__
```
일반 모드에서 @a라고 입력하면 a가 재생된다.
@@를 입력하면 제일 마지막에 재생된 매크로, 즉 가장 최근에 @e를 했다면 @@떄 재생되는 매크로는 e가 된다.

+10@a를 입력하면 a를 10번 실행한다.
```

* __자주 사용하는 매크로 등록 방법__
```
1. ~/.vimrc를 연다.
2. let @a='매크로 문자열' <- 이런식으로 매크로로 동작시킬 문자열을 입력
```

* __매크로 문자열을 편집기에 그대로 출력하는 방법__
```
-첫 번째 방법: 입력 모드에서 ctrl + r, ctrl + r, 매크로 문자를 순서대로 입력하면 그대로 출력됨
-두 번째 방법: 일반 모드에서 "매크로문자p를 입력하여 래지스터로부터 바로 붙여넣기 한다. (ex) "ap, "bp, "cp...
